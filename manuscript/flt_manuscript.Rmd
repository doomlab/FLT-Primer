...
The result from the `hunspell()` function is a list object of spelling errors for each row of data. For example, when responding to `r toupper(X$cue[173])`, a participant wrote <*`r trimws(gsub("  ", " ", X$feature_response[173]))`*>, and the spelling errors were denoted as <*`r paste0(spelling.errors[grep("yelloe|cara", spelling.errors)], collapse = "> <")`*>. After checking for errors, the `hunspell_suggest()` function was used to determine the most likely replacement for each error. For <*yelloe*>, both <*`r paste0(unlist(spelling.sugg[[38]]), collapse = "> <")`*> were suggested, and <*`r paste0(unlist(spelling.sugg[[39]]), collapse = "> <")`*> were suggested for <*caramell*>. 
