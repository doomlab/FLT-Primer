...
b_map <- unique(b_map)
b_map <- b_map[b_map$cue %in% master$word , ]
b_table <- table(b_map$pos_cue)

## Importing concreteness ratings
concrete <- read.delim("../data/concrete.txt")
...
You can load the entire set of libraries for this tutorial using the `dependencies.R` script found in our online repository, as shown below^[A `packrat` project compliation is avaliable on GitHub for reproducibility [@Ushey2018]]. 
...
For *`r spelling_errors[[173]][1]`*, both *`r paste0(unlist(spelling_suggest[[173]][1]), collapse = " ")`* were suggested, and *`r paste0(unlist(spelling_suggest[[173]][2]), collapse = " ")`* were suggested for *`r spelling_errors[[173]][2]`*. The suggestions are presented in most probable order, and using a few loops with the substitute (`gsub()`) function, we can replace all errors with the most likely replacement in a new dataset `spell_checked`. A specialized dictionary with pre-coded error responses and corrections could be implemented at this stage. Other paid alternatives, such as Bing Spell Check, can be a useful avenue for datasets that may contain brand names (i.e, *apple* versus *Apple*) or slang terms and provides context sensitive corrections (e.g., keeping *Apple* as a response to computer, but not as a response to green).
...
Similar to spelling correction `stri_replace_all_regex()`  is used to replace the wordforms with their corresponding lemmas from the `stringi` package [@Gagolewski2019]. Table \@ref(tab:tab3) shows the processed data at this stage. 
...
```{r strudel, include = F}
strudel <- read.csv("../data/baked.strudel.csv", stringsAsFactors = F)
